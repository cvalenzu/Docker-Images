...
#Writer: MaxdSre
#Date: Sep 22, 2017 16:59 Fri +0800
#Update Time:
...
funcVersionOnlineInfo(){
    ...
    # downloads['jdk-9-oth-JPR']['files']['jdk-9_linux-x64_bin.tar.gz'] = { "title":"Linux", "size":"330.23 MB","filepath":"http://download.oracle.com/otn-pub/java/jdk/9+181/jdk-9_linux-x64_bin.tar.gz", "MD5":"abe68b8ba280d11cb8f937410543750c", "SHA256":"1c6d783a54fcc0673ed1f8c5e8650b1d8977ca3e856a03fba0090198e0f16f6d"};

    $download_tool "${oracle_java_se_page}" | sed -r -n '/<h1>Java SE Downloads/{s@<\/?(td|br)[^>]*>@\n\n@g;p}' | sed -r -n '/id="javasejdk"><a name=/!d;s@<\/h[^>]*>[[:space:]]*@@g;s@.*href="([^"]*)">([^<]*)<[^>]*>(.*)$@\2|\1|\3@g;p' | awk -F\| -v official_site="${oracle_official_site}" 'BEGIN{OFS="|"}{major_version=gensub(/[^[:digit:]]*([[:digit:]]{1}).*/,"\\1","g",$1); if($2!~/^https?/){$2=official_site""$2}; print major_version,$1,$2,$3}' | while IFS="|" read -r major_version version_name download_page_url simple_intorduction; do
        # 8|Java SE 8u144|http://www.oracle.com/technetwork/java/javase/downloads/jdk8-downloads-2133151.html|Java SE 8u144 includes important bug fixes. Oracle strongly recommends that all Java SE 8 users upgrade to this release.

        local specific_jdk_info=${specific_jdk_info:-}
        specific_jdk_info=$($download_tool "${download_page_url}" | sed -r -n '/linux-x64.*tar.gz/{/demo/d;s@.*=@@g;s@(\{|\})@@g;s@,@\n@g;p}' | sed -r -n 's@[^:]+:[^"]*"([^"]*)".*@\1@g;s@\n@|@g;p' | sed ':a;N;$!ba;s@\n@|@g')
        # title | size | filepath | md5 |sha256
        # Linux x64|176.92 MB|http://download.oracle.com/otn-pub/java/jdk/8u144-b01/090f390dda5b47b9b721c7dfaa008135/jdk-8u144-linux-x64.tar.gz|2d59a3add1f213cd249a67684d4aeb83|e8a341ce566f32c3d06f6d0f0eeea9a0f434f538d22af949ae58bc86f2eeaae4

        l_pack_download_link=${l_pack_download_link:-}
        l_pack_download_link=$(echo "${specific_jdk_info}" | awk -F\| '{print $3}')
        l_pack_name=${l_pack_name:-}
        l_pack_name="${l_pack_download_link##*/}"
        l_pack_sha256_dgst=${l_pack_sha256_dgst:-}
        l_pack_sha256_dgst=$(echo "${specific_jdk_info}" | awk -F\| '{print $NF}')
        # major_version|version_name|download_page_url|simple_intorduction|pack_name|pack_download_link|pack_sha256_dgst
        echo "${major_version}|${version_name}|${download_page_url}|${simple_intorduction}|${l_pack_name}|${l_pack_download_link}|${l_pack_sha256_dgst}" >> "${oracle_jdk_info}"

    done    # end while

    # 9|Java SE 9|http://www.oracle.com/technetwork/java/javase/downloads/jdk9-downloads-3848520.html|Java SE 9 is the latest update to the Java Platform. This release includes much awaited new features like the modularization of the Java Platform, better performance, support for new standards, and many other improvements. |jdk-9_linux-x64_bin.tar.gz|http://download.oracle.com/otn-pub/java/jdk/9+181/jdk-9_linux-x64_bin.tar.gz|1c6d783a54fcc0673ed1f8c5e8650b1d8977ca3e856a03fba0090198e0f16f6d
    # 8|Java SE 8u144|http://www.oracle.com/technetwork/java/javase/downloads/jdk8-downloads-2133151.html|Java SE 8u144 includes important bug fixes. Oracle strongly recommends that all Java SE 8 users upgrade to this release.|jdk-8u144-linux-x64.tar.gz|http://download.oracle.com/otn-pub/java/jdk/8u144-b01/090f390dda5b47b9b721c7dfaa008135/jdk-8u144-linux-x64.tar.gz|e8a341ce566f32c3d06f6d0f0eeea9a0f434f538d22af949ae58bc86f2eeaae4
    ...
    if [[ "${version_check}" -eq 1 ]]; then
        ...
        awk -F\| 'BEGIN{OFS="|"}{print $2,$4}' "${oracle_jdk_info}" | while IFS="|" read -r version_name simple_intorduction; do
            printf "${c_blue}%s${c_normal}: %s\n\n" "${version_name}" "${simple_intorduction}"
        done
        exit
    fi
...
funcVersionSelectionMenu(){
    ...
    # download_page_url=$(awk -F\| 'match($2,/^'"${choose_version}"'$/){print $6}' "${oracle_jdk_info}")
    # version_simple_introduce=$(awk -F\| 'match($2,/^'"${choose_version}"'$/){print $4}' "${oracle_jdk_info}")
}
...
funcDownloadAndDecompressOperation(){
    ...
    if [[ "${source_pack_path}" =~ .tar.gz$ && -s "${source_pack_path}" ]]; then
        ...
        pack_sha256_dgst=$(awk -F\| 'match($5,/^'"${local_pack_name}"'$/){print $7}' "${oracle_jdk_info}")
    ...
    # 1.2 - manually specify jdk major version
        ...
        pack_name=$(awk -F\| 'match($1,/^'"${version_specify}"'$/){print $5}' "${oracle_jdk_info}")
        pack_download_link=$(awk -F\| 'match($1,/^'"${version_specify}"'$/){print $6}' "${oracle_jdk_info}")
        pack_sha256_dgst=$(awk -F\| 'match($1,/^'"${version_specify}"'$/){print $7}' "${oracle_jdk_info}")
    ...
    # 1.3 - version choose from selection menu list
        ...
        if [[ -n "${choose_version}" ]]; then
            pack_name=$(awk -F\| 'match($2,/^'"${choose_version}"'$/){print $5}' "${oracle_jdk_info}")
            pack_download_link=$(awk -F\| 'match($2,/^'"${choose_version}"'$/){print $6}' "${oracle_jdk_info}")
            pack_sha256_dgst=$(awk -F\| 'match($2,/^'"${choose_version}"'$/){print $7}' "${oracle_jdk_info}")
        fi    # end if
